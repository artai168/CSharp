...
namespace TopStar_Invoice_Maker_SQLSever.Views.UI.Invoice
{
    ...
    public partial class UC_Invoice : UserControl
    {
        ...
        private void loadInvoiceHeader(Models.Invoice inInvoice)
        {
            ...
            this.txtExtraDiscount.Text = inInvoice.Discount; //ExtraDiscount
        ...
        private void txtClinicID_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            foreach (Models.Clinic temp_clinic_Detail in myLINQResult)
            {
                ...
                this.txt_Discount.Text = temp_clinic_Detail.Discount;
            }
        }
        ...
        {
            string str_SubTotal = Convert.ToString(caculate_SubTotal_Order_Amount());
            string str_Total = the_Final_Total(str_SubTotal, this.txtExtraDiscount.Text);
            ...
            return new Models.Invoice
            {
                ...
                Discount = this.txtExtraDiscount.Text,
        ...
        private void load_new_Product_List()
        {
        ...
        {
            if ((Keyboard.Modifiers == ModifierKeys.Control && e.Key == Key.L) || (e.Key == Key.Enter))
            {
                add_to_InvoiceItem();
            }
            else if (Keyboard.Modifiers == ModifierKeys.Control && e.Key == Key.S)
        ...
        private decimal caculate_SubTotal_Order_Amount()
        {
        ...
        private decimal caculate_Total_Order_Amount()
        {
            ...
            if (!string.IsNullOrEmpty(this.txtExtraDiscount.Text))
            {
                totalAmount = totalAmount - Convert.ToDecimal(this.txtExtraDiscount.Text);
            }
        ...
        private void txtNetPrice_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (!string.IsNullOrWhiteSpace(this.txtNetPrice.Text))
            {
                ...
                if (str_Proccessing.IndexOf("=") == 0)
                {
                    ...
                    int myResult = (int)Math.Ceiling(Convert.ToDecimal(value));
                    this.txtNetPrice.Text = Convert.ToString(myResult);
                }
            }
            else
            {
                if (MessageBox.Show("Do you want to Search & Load the pervious price of this customer?", "Confirm",
                    MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
                {
                    //Load pervious price
                    Controllers.InvoiceController invoiceController = new Controllers.InvoiceController();
                    this.txtNetPrice.Text = invoiceController.get_Last_Price(this.txtClinicID.Text, this.txtProductID.Text);
                }
                else
                {
                    if ((!string.IsNullOrEmpty(this.txt_Discount.Text.Trim())) && (!string.IsNullOrEmpty(this.txtUnitPrice.Text.Trim())))
                    {
                        decimal cost = Convert.ToDecimal(this.txtUnitPrice.Text.Trim());
                        decimal discount = (Convert.ToDecimal(this.txt_Discount.Text)) / 100;
                        decimal totalAmount = cost * discount;
                        this.txtNetPrice.Text = Convert.ToString(totalAmount);
                    }
        ...
        private void txtExtraDiscount_LostFocus(object sender, RoutedEventArgs e)
        {
            if (!string.IsNullOrEmpty(this.txtExtraDiscount.Text))
            {
                string expression = @"^(\d+|\d+[.]\d+)%";
                Regex objNotNumberPattern = new Regex(expression);
                if (objNotNumberPattern.IsMatch(this.txtExtraDiscount.Text))
                {
                    checkDiscount(this.txtExtraDiscount.Text.Trim('%'));
                }
            }
        ...
        private void checkDiscount( string discountRate)
        {
            var discount = Convert.ToDecimal(discountRate);
            var discountedAmount = caculate_SubTotal_Order_Amount() * (discount/100);
            this.txtExtraDiscount.Text = discountedAmount.ToString();
        }
        ...
        private void lstInvoiceItem_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Delete)
            {
